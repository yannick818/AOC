...
pub fn cal_cubes(input: &str) -> Result<usize> {
    let sum = input.lines().enumerate().filter_map(|(id, line)| {
        match is_possible(line) {
            Ok(true) => Some(id+1),
            _ => None
        }
    })
...
fn is_possible(game: &str) -> Result<bool> {
    ...
    let max_cubes = games.trim().split(';').fold((0_u32, 0_u32, 0_u32), |mut max, round| {
        round.split(',')
        .map(|r| r.trim())
    ...
    match (max_cubes.0 > MAX_RED, max_cubes.1 > MAX_GREEN, max_cubes.2 > MAX_BLUE) {
